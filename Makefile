...
get_init_data:
	python ./src/feature_store/generate_initial_data.py ./src/config/feature_store/config.yml

# Preprocess and transform data before ingestion by feature store
prep_data:
	python ./src/feature_store/prep_data.py  ./src/config/feature_store/config.yml
...
split_data:
	python ./src/training/split_data.py ./src/config/training/config.yml

train:
	python ./src/training/train.py ./src/config/training/config.yml

evaluate:
	python ./src/training/evaluate.py ./src/config/training/config.yml

submit_train: prep_data split_data train evaluate

# Test model locally
test_model:
	python ./src/inference/predict.py ./src/config/training/config.yml
